...
fn compute_energized(
    ...
    let mut worklist = vec![(starting_pos, starting_direction)];
    ...
    energized
        .into_iter()
        .map(|row| row.into_iter().map(|v| v.iter().any(|&b| b)))
        .flatten()
        .filter(|&b| b)
        .count() as i32
}
...
fn main() {
    let map = get_map(&read_file());
    let mut res = 0;
    for y in 0..map.len() {
        res = res.max(compute_energized(
            Position { y: y as i32, x: 0 },
            Direction::Right,
            &map,
        ));
        res = res.max(compute_energized(
